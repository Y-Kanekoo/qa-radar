...
| ~~P3~~ | ~~`weight_tags_text` が YAML にあるが未実装(デッドコンフィグ)~~(2026-08-03 解消: `weight_tags_text` を削除し冒頭コメントを実態に修正)。「タグ 0 件は LLM フォールバック」コメントも未実装 | `config/tag_rules.yaml:4,9` |
...
- ~~(PR #18 レビューでの積み残し、優先度低) `httpx.TransportError` を毎回律儀にリトライしており、
  ...
  (2026-08-03 解消: `UnsupportedProtocol` / `LocalProtocolError` 等の恒久エラーをリトライ対象外に変更)
